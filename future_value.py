...
def main():
    # future_value present_value interest no_comp years - 4 arguments
    # FV = PV * [1+(i/n)]^(n*t)
    ...
    # python future_value.py 10000 10 1 5

    print (sys.argv)
    if len(sys.argv) == 5:    
        present_value = int( sys.argv[1] )
        interest = int( sys.argv[2]  )
        no_comp = int( sys.argv[3] )
        years = int( sys.argv[4] )
        print ('Present Value = ', present_value, 'Interest % = ', interest, 'Number of Compounding periods in a year = ', no_comp, 'Years  = ', years)
        ...
        print('exp = ',  exp)
        future_value = present_value * ( exp)
        print('future value = ', future_value)
    else:
        print('incorrect usage - future_value.py present_value interest no_comp years - 5 arguments')
